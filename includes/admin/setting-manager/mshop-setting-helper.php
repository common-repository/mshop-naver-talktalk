...
if ( ! class_exists( 'MSNTT_SettingHelper' ) ) {
	class MSNTT_SettingHelper {
		public static function get_settings( $setting, $postid = null ) {
			...
			if ( ! empty( $setting['id'] ) && ! in_array( $setting['type'], array( 'Tab', 'Page' ) ) ) {
				if ( ! empty( $setting['readonly'] ) && 'yes' == $setting['readonly'] ) {
					$value = ! empty( $setting['default'] ) ? $setting['default'] : '';
				} else {
			...
			if ( ! empty( $setting['id'] ) ) {
				if ( ! empty( $setting['readonly'] ) && 'yes' == $setting['readonly'] ) {
					...
					if ( ! empty( $_REQUEST[ $setting['id'] ] ) ) {
						$postid ? update_post_meta( $postid, $setting['id'], sanitize_text_field( $_REQUEST[ $setting['id'] ] ) ) : update_option( $setting['id'], sanitize_text_field( $_REQUEST[ $setting['id'] ] ) );
					} else {
		...
		public static function get_setting_values( $setting ) {
			...
			if ( ! empty( $setting['id'] ) ) {
				if ( ! empty( $setting['readonly'] ) && 'yes' == $setting['readonly'] ) {
					...
					$values[ $setting['id'] ] = sanitize_text_field( $_REQUEST[ $setting['id'] ] );
